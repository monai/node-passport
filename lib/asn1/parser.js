...
class Parser extends EventEmitter {
  ...
  read(chunk) {
    ...
    do {
      if (this.chunkOffset < this.chunk.length && this.stack.length === 0) {
        ...
        this.emit('data', { stack: this.constructor.push });
      }
  ...
  readContents(chunk) {
    ...
    if (current.contentsRead <= current.length && this.chunkOffset < this.chunk.length) {
      this.stack.push(this.constructor.createDataValue());
      this.emit('data', { stack: this.constructor.push });
    }
  }
  ...
  collapse() {
    ...
    if (current.tag.identifier === 0x00 && previous?.length === Infinity) {
      this.stack.pop();
      this.stack.pop();
      this.emit('data', { stack: this.constructor.pop, eoc: true });
    ...
    while (current && current.length === current.contentsRead) {
      this.stack.pop();
      this.emit('data', { stack: this.constructor.pop });
