...
const deriveKey = require('../liteid/deriveKey');
...
async function performPace(reader, options) {
  ...
  //   0x01: MRZ
  //   0x02: CAN
  capdu = new CommandApdu(0x00, 0x22, 0xc1, 0xa4, { data: '800a04007f00070202040201830102' });
  response = await reader.transmit(capdu);
  assertSwOk(response);
  ...
  capdu = new CommandApdu(0x10, 0x86, 0x00, 0x00, { data: '7c00', le: 0x0c });
  ...
  const encryptedNonce = response.data.slice(4);
  ...
  capdu = new CommandApdu(0x10, 0x86, 0x00, 0x00, { le: 0x45, data });

  // 7C: 82: Mapping Data
  response = await reader.transmit(capdu);
  assertSwOk(response);

  const mapPkIc = response.data.slice(4);
  ...
  const ephPkIc = response.data.slice(4);
  ...
  capdu = new CommandApdu(0x00, 0x86, 0x00, 0x00, { le: 0x0c, data });
