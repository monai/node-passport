...
const keyAgreements = {
  [dh]: {
    ...
    createKeyPair({ prime, generator }) {
      return crypto.createDiffieHellman(prime, generator);
    },
  },
  [ecdh]: {
...
async function performPace(reader, options) {
  ...
  if (options.parameterId === 10 && imIdentifiers.includes(currentConf.mappingIdentifier)) {
    throw new Error(`Curve ${standardizedDomainParameters[10].curve} cannot be used with the Integrated Mapping`);
  }
