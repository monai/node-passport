...
import { ecdh as _ecdh } from '@monai/pace';
import CommandApdu from '../iso7816/command_apdu.js';
import Session from './session.js';
import { pad, mac } from '../iso9797.js';
import { kdf } from './kdf.js';
import { assertSwOk } from '../iso7816/util.js';
import { ab2aba, hex2buf } from '../util.js';
import tlv from '../asn1/tlv.js';
import { encode as encodeOid } from '../asn1/codecs/object_identifier.js';
...
export const mappings = {
  ...
  [ecdhGm]: {
    ...
    identifier: 'ECDH-GM',
    map: _ecdh.gm.map,
    generateKeys: _ecdh.gm.generateKeys,
  },
  [dhIm]: {
    arc: dhIm,
    keyAgreement: dh,
...
export async function performPace(reader, options) {
  ...
  if (!('protocol' in options)) {
    throw new TypeError('Protocol OID is missing');
  }

  const currentConf = {
    ...conf[options.protocol],
    ...standardizedDomainParameters[options.parameterId],
  };
  ...
  if (currentConf.noIm && imIdentifiers.includes(currentConf.mappingIdentifier)) {
    const { curve } = standardizedDomainParameters[options.parameterId];
  ...
  const decipher = createDecipheriv(currentConf.algorithm, nonceKey, Buffer.alloc(8));
  ...
  const tPcd = currentConf.mac(kMac, tMsg);
