...
const ResponseApdu = require('../iso7816/response_apdu');
...
function unprotectResponseApdu(session, rapdu) {
  ...
  // TODO: consider moving to the decrypt utility
  const ciphertext = do87.contents.slice(1);
  const ciphertextAligned = ciphertext.length === 8
    ? Buffer.concat([ciphertext, Buffer.alloc(8)])
    : ciphertext;

  const data = do87 && unpad(session.decrypt(ciphertextAligned));
  return new ResponseApdu(data, do99.contents);
}

function unwrap(data) {
  return parse(data).children.reduce((acc, node) => {
