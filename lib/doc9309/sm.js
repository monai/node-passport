...
function unprotectCommandApdu(session, apdu) {
  ...
  const commandHeader = apdu.getEncodedHeader();

  if (do8e) {
    const CC = do8e.contents;

    const M = Buffer.concat([pad(commandHeader), do87 && do87.data].filter(Boolean));
    session.increment();
    const N = pad(Buffer.concat([session.ssc, M]));
  ...
  const contents = do87 ? unpad(session.decrypt(do87.contents.slice(1))) : Buffer.alloc(0);

  return new CommandApdu(
    0x00,
    ...commandHeader.slice(1, 4),
    {
      data: contents,
      le: do97 && do97.contents,
    },
  );
}
