...
function unprotectCommandApdu(session, capdu) {
  ...
  if (do8e) {
    ...
    const M = Buffer.concat([pad(cmdHeader), do87?.contents, do97?.contents].filter(Boolean));
    session.increment();
    const N = pad(Buffer.concat([session.ssc, M]));
...
function unprotectResponseApdu(session, rapdu) {
  ...
  if (do8e) {
    ...
    session.increment();
    const K = pad(Buffer.concat([session.ssc, do87?.contents, do99.contents].filter(Boolean)));
