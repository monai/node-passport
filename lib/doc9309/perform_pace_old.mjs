...
import { kdfPi, kdfEnc, kdfMac } from './kdf.mjs';
...
export async function performPace(reader, options) {
  ...
  let nonceKey = kdfPi(currentConf.algorithm, options.password);
  ...
  const kEnc = kdfEnc(currentConf.algorithm, sharedSecret);
  const kMac = kdfMac(currentConf.algorithm, sharedSecret);
