...
function unprotect(session, rapdu) {
  ...
  if (rapdu.data[0] === 0x87) {
    const [encDataLength, o] = asn1Length(rapdu.data.slice(1));
    offset = 1 + o;
  ...
  if (rapdu.data[offset] === 0x8e) {
    ...
    // const do8E = rapdu.data.slice(offset, offset + 2 + ccLength);
...
function unpadAndDecryptData(key, data) {
  ...
  key = Buffer.concat([key, key.slice(0, 8)]);
  const decipher = forge.cipher.createDecipher('3DES-CBC', toForgeBuffer(key));
  decipher.start({ iv: toForgeBuffer(iv) });
  decipher.update(toForgeBuffer(data));

  return unpad(toBuffer(decipher.output));
}
