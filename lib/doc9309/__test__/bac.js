...
const { kdf, adjustParity } = require('../kdf');

t.test('Doc 9309-11 Appendix D', (t) => {
  ...
  t.test('D.2 Derivation of Document Basic Access Keys (Kenc and Kmac)', (t) => {
    ...
    const [kEnc, kMac] = bac.computeBacKeys(mrzInfo);

    const kEncAdj = adjustParity(kEnc);
    const kMacAdj = adjustParity(kMac);

    t.assert(kEncAdj.equals(Buffer.from(exptEnc, 'hex')), 'Kenc');
    t.assert(kMacAdj.equals(Buffer.from(exptMac, 'hex')), 'Kmac');
  ...
  t.test('D.3 Authentication And Establishment of Session Keys', (t) => {
    ...
    t.test('computeEifd', (t) => {
      ...
      const computedEifd = bac.computeEifd(kEnc, s);

      t.assert(computedEifd.equals(eIfd), 'eIfd');
    ...
    t.test('computeSessionKeys', (t) => {
      const sessionKeys = bac.computeSessionKeys({
      ...
      t.assert(ksEnc.equals(ksEncAdj), 'ksEnc');
      t.assert(ksMac.equals(ksMacAdj), 'ksMac');

      t.end();
    });

    t.test('computeSsc', (t) => {
      const computedSsc = bac.computeSsc({ rndIfd, rndIc });

      t.assert(ssc.equals(computedSsc), 'ssc');
