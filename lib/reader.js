const util = require('util');
const pcsc = require('@pokusew/pcsclite');

const debuglog = util.debuglog('reader');

class Reader {
  constructor() {
    const pl = pcsc();

    this.pl = pl;
    this.reader = undefined;
    this.state = undefined;
    this.atr = undefined;
    this.protocol = undefined;

    pl.on('reader', this.onReader.bind(this));
  }

  close() {
    this.reader.close();
    this.pl.close();
  }

  onReader(reader) {
    this.reader = reader;
    this.readerConnect = reader.connect.bind(reader);
    this.readerConnectP = util.promisify(this.readerConnect);
    this.readerTransmit = reader.transmit.bind(reader);
    this.readerTransmitP = util.promisify(this.readerTransmit);

    reader.on('error', onReaderError);
    reader.on('status', this.onReaderStatus.bind(this));
  }

  onReaderStatus(status) {
    this.state = status.state;
    this.atr = status.atr;
  }

  isCardPresent() {
    if (this.state === undefined || !this.reader) {
      return false;
    }

    return !!(this.state & this.reader.SCARD_STATE_PRESENT);
  }

  waitForCard(timeout = 2000) {
    return new Promise((resolve, reject) => {
      const start = Date.now();

      if (this.isCardPresent()) {
        resolve();
      } else {
        const handler = () => {
          if (this.isCardPresent()) {
            resolve();
          } else if (Date.now() - start < timeout) {
            setTimeout(handler, 100);
          } else {
            reject(new Error('wait for card timeout'));
          }
        };
        handler();
      }
    });
  }

  async connect(options) {
    if (this.protocol !== undefined) {
      return this.protocol;
    }

    const protocol = await this.readerConnectP(options);
    this.protocol = protocol;
    return protocol;
  }

  transmit(message, bl) {
    if (!this.protocol) {
      throw new Error('not connected');
    }
    return this.readerTransmitP(message, bl, this.protocol);
  }
}

module.exports = Reader;

function onReaderError(error) {
  debuglog('onReaderError', error);
}
