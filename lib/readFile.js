...
const debuglog = util.debuglog('read_file');
...
function createReadStream(options) {
  ...
  if (!['shortFileId', 'select'].includes(options.mode)) {
  ...
  service.onTransition((ctx, event) => {
    debuglog(`${event.type}: ${ctx.value}`);
  });
  service.start();
...
async function selectFileP(ctx) {
  let offset = 0;
  if (ctx.mode === 'shortFileId') {
    offset = (0x80 ^ ctx.shortFileId) << 8;
...
async function readHeaderP(ctx) {
  let res;
  let eof = false;
