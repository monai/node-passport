...
class Iso7816Error extends Error {
  constructor(sw1, sw2) {
    ...
    const errorNumber = `0x${[sw1, sw2].map((number) => formatHex(number)).join('')}`;
    let message;
    try {
      message = errors[sw1];
      message = typeof message === 'function' ? message(sw1, sw2) : message[sw2];
    // eslint-disable-next-line no-empty
    } catch {}
    message = message ? `${message} [${errorNumber}]` : `unknown error [${errorNumber}]`;
    super(message);
  }
}
